...
public class Subscriber extends BasePubSub {
    ...
    public Subscriber(int lookupIntervalSecs, int maxLookupFailuresBeforeError, String... lookupHosts) {
        this(Client.getDefaultClient(), lookupIntervalSecs, maxLookupFailuresBeforeError, lookupHosts);
    }
        ...
        for (HostAndPort lookup : lookups) {
            ...
            try {
                ...
                if (con.getResponseCode() != 200) {
                    log.debug("ignoring lookup resp:{} nsqlookupd:{} topic:{}", con.getResponseCode(), lookup, topic);
                    continue;
                }
                ...
                if (lookupFailureCount >= this.maxLookupFailuresBeforeError) {
                    log.error("lookup failure. lookup failed for {} consecutive tries. nsqlookupd:{} topic:{}",
                            lookupFailureCount, lookup, topic, e);
                } else {
                    log.warn("lookup failure. lookup failed for {} consecutive tries. nsqlookupd:{} topic:{}",
                            lookupFailureCount, lookup, topic, e);
                }
            } finally {

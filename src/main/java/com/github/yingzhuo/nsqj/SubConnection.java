...
class SubConnection extends Connection {
    ...
    public synchronized void finish(String id) {
        try {
            ...
            log.error("finish error. {}", stateDesc(), e);
            close();
        }
    }
    ...
    public synchronized void requeue(String id) {
        try {
            ...
            log.error("requeue error. {}", stateDesc(), e);
            close();
        }
    }
    ...
    public synchronized void touch(String id) {
        try {
            ...
            log.error("touch error. {}", stateDesc(), e);
            close();
        }
    }
    ...
    private synchronized void delayedFlush() {
        try {
            ...
            log.error("delayedFlush error. {}", stateDesc(), e);
            close();
        }
    }
    ...
    public synchronized void setMaxInFlight(int maxInFlight, boolean isActive) {
        try {
            ...
            log.debug("RDY:{} {}", maxInFlight, toString());
            ...
            log.error("setMaxInFlight failed. con:{}", stateDesc(), e);
            close();
        }
    }
            ...
            handlerExecutor.execute(new Runnable() {
                public void run() {
                    try {
                        ...
                        log.error("failed message error", t);
                    }
                }
            });
        }
